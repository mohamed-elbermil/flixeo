...
export const tmdbService = {
   ...
   */
  async fetchTMDB<T>(endpoint: string, params: Record<string, string | number> = {}): Promise<T> {
    if (!API_KEY) {
   ...
   */
  async getTrending(type: 'movie' | 'tv' | 'all' = 'all', timeWindow: 'day' | 'week' = 'week', page = 1): Promise<TMDBResponse<Movie>> {
    return this.fetchTMDB<TMDBResponse<Movie>>(`/trending/${type}/${timeWindow}`, { page });
  },
  ...
  async search(query: string, page = 1): Promise<TMDBResponse<Movie>> {
    return this.fetchTMDB<TMDBResponse<Movie>>('/search/multi', { query, page });
  },

  /**
   * Get genres list
   */
  async getGenres(type: 'movie' | 'tv' = 'movie'): Promise<{ genres: Genre[] }> {
